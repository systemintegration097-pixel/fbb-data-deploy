...
function initFilters() {
    ...
    if (rBranch && rZone) {
        rBranch.addEventListener('change', async () => {
            ...
            if (branchVal) {
                try {
                    const response = await fetch(`/api/branches/${encodeURIComponent(branchVal)}/zones`);
...
async function loadPartnerCapacityReport() {
    try {
        ...
        const resPartners = await fetch('/api/partners/capacity');
        ...
        const resZones = await fetch('/api/zones/capacity-detail');
...
async function toggleKMLLayer() {
    ...
    if (checkbox.checked) {
        ...
        try {
            const response = await fetch('/static/coverage.geojson');
            const data = await response.json();
...
async function searchNearestBoxes(lat, lng) {
    ...
    try {
        const response = await fetch(`/api/boxes/nearest?latitude=${lat}&longitude=${lng}&limit=12`);
        const data = await response.json();
...
async function saveZone() {
    ...
    const url = isEdit ? `/api/zones/${zoneId}` : '/api/zones';
...
async function loadStackedCapacityChart(branch = '', partner = '') {
    try {
        ...
        const response = await fetch(`/api/charts/branch-capacity-stacked?${params.toString()}`);
        const data = await response.json();
...
async function loadIncidentsMonthsOptions() {
    try {
        ...
        const response = await fetch('/api/incidents/months');
...
async function loadDeploymentsMonthsOptions() {
    try {
        ...
        const response = await fetch('/api/deployments/months');
        const months = await response.json();
...
async function loadIncidentsWeeksOptions() {
    try {
        ...
        const response = await fetch('/api/incidents/weeks');
...
async function loadIncidentsSiteOptions(branch = '') {
    try {
        ...
        const response = await fetch(`/api/incidents/sites?branch=${encodeURIComponent(branch)}`);
...
async function loadIncidents() {
    try {
        ...
        const response = await fetch(`/api/incidents/stats?${params.toString()}`);
        const result = await response.json();
...
async function loadSiteOutagesReport(branch, month, site, week) {
    try {
        ...
        const response = await fetch(`/api/incidents/outages?${params.toString()}`);
        const result = await response.json();
...
async function showOutageDetailsModal(site) {
    try {
        ...
        const response = await fetch(`/api/incidents/outages/details?${params.toString()}`);
...
async function showWoDetailsModal(site) {
    try {
        ...
        const response = await fetch(`/api/incidents/wos/details?${params.toString()}`);
        const details = await response.json();
        ...
        if (details.length === 0) {
            ...
            details.forEach(item => {
                ...
                const recBadge = isRec 
                    ...
                    : `<span style="padding: 2px 6px; border-radius: 4px; font-size: 11px; font-weight: 700; background-color: rgba(107,114,128,0.2); color: #9ca3af;">No</span>`;
...
async function loadDeploymentsReport() {
    ...
    try {
        ...
        const response = await fetch(`/api/deployments/stats?${params.toString()}`);
        const data = await response.json();
        ...
        AppState.charts.deployPartnerEfficiency = new Chart(effContext, {
            ...
            options: {
                ...
                scales: {
                    ...
                    y: {
                        grid: { display: false },
                        ticks: { color: '#9ca3af', font: { family: 'Inter', size: 9 }, autoSkip: false }
                    }
                }
            }
        });
