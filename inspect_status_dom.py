...
print("Inspeccionando opciones reales del dropdown de WO status...")
with sync_playwright() as p:
    ...
    # Abrir dropdown
    print("Abriendo dropdown de WO status...")
    dropdown_container = page.locator("div:has(> #custom-statusSearchWeb)").first
    dropdown_container.click(force=True)
    page.wait_for_timeout(3000)
    
    # Imprimir todos los elementos de tipo div que sean descendientes del body y tengan pinta de ser la lista
    print("\nBuscando todas las opciones de dropdown en la página...")
    # Buscamos divs que contengan texto y estén posicionados en la lista de opciones
    ...
    for div in all_divs:
        try:
            ...
            if class_name and ("option" in class_name.lower() or "menu" in class_name.lower() or "react-select" in class_name.lower()):
                text = div.inner_text().strip()
                if text and len(text) < 50 and text not in options_found:
                    options_found.append(text)
                    print(f"  [Div Class: {class_name}] -> '{text}'")
        except:
            continue
            
    # También buscamos por el rol de opción
    role_options = page.locator("[role='option']").all()
    for opt in role_options:
        try:
            text = opt.inner_text().strip()
            print(f"  [Role Option] -> '{text}'")
        except:
            continue
            
    # Guardar captura de pantalla del dropdown abierto para inspección visual directa
    screenshot_path = "C:\\Users\\jjvar\\.gemini\\antigravity-ide\\brain\\59ce769f-dd63-46e1-bb61-e10fb7442f08\\screenshot_dropdown_inspected.png"
