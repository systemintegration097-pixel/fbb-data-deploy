...
def run_download():
    with sync_playwright() as p:
        ...
        # 3. Ensure Wait_for_accept is checked in WO Status
        print("Asegurando Wait_for_accept en WO Status...")
        wait_cb = frame.locator("input[name*='Calculation_1551771557234716675'][name$='_2']").first
        if wait_cb.count() > 0:
            wait_cb.check(force=True)
            time.sleep(3)

        print("\n=== ESTADO DE FILTROS APLICADOS ===")
        print("Status Recv RECOVERING checked:", rec_cb.is_checked() if rec_cb.count()>0 else "N/A")
        print("WO Status Processing checked:", proc_cb.is_checked() if proc_cb.count()>0 else "N/A")
        print("WO Status Wait_for_accept checked:", wait_cb.is_checked() if wait_cb.count()>0 else "N/A")

        print("\nEsperando 12s para refrescar datos en la grilla de Tableau...")
        time.sleep(12)
        
        page.screenshot(path="scratch_tableau_with_processing.png")
...
def verify():
    ...
    print(f"\n--- VERIFICACIÓN DE MUESTRA DE WOs EN NUEVO REPORTE ({len(tab_rows)} filas) ---")
    ...
    print(f"Encontradas {found_count} de {len(sample_wos)} en el nuevo Excel de Tableau.")
